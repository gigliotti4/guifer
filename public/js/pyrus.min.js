    ...
    this.elements = ( target , url , data , buttonsOK = [ "c" , "e" , "d" ] , button = null ) => {
        ...
        dataAUX.forEach( function( x ) {
            ...
            this.columnas().forEach( function( column ) {
                ...
                let flag = 1;
                if (this.objeto.IDIOMAS) {
                    if (this.objeto.IDIOMAS[ column.COLUMN ]) {
                        flag = 0;
                        for( let t in this.objeto.IDIOMAS[ column.COLUMN ].ELEMENT ) {
                            ...
                            row.push(`<td style="vertical-align: top !important;" class="${column.CLASS}">${td}</td>`);
                        }
                    }
                }
                if (flag) {
                    if(this.especificacion[ column.COLUMN ].TIPO == "TP_STRING")
                        row.push(`<td class="${column.CLASS}"><span class="edit" data-name="${this.name}" data-column="${column.COLUMN}" data-id="${id}">${element}</span></td>`);
                    else
